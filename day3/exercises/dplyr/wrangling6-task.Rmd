...
library(here)
library(skimr)
tidy_nails <- here("data", "tidy_nails.csv") %>% 
  read_csv()
tidy_nails
```
...
```{r gather, error=TRUE}
gather_nails <- tidy_nails %>%
  gather( key = "measure", value = "count", technicians, salons)  %>% 
  arrange( desc(state) & desc(year) )
