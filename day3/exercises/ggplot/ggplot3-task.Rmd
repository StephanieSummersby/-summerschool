...
geom_boxplot() +
theme_bw()+
facet_wrap(~reorder(season_name, season))+
  xlab("Season")
```
...
ggplot(
  data = beaches,
  mapping = aes(
    reorder(season_name, season),
    x = season_name,
    y = log10(enterococci)
  ) 
) + 
geom_boxplot() +
  geom_jitter(alpha=0.5, aes(colour = season_name)) +
theme_bw()+
...
theme_bw()+
  ...
  facet_wrap(~reorder(season_name, season))+
labs(
  title = "Enterococci levels by temperature",
  subtitle = "Varying by season",
  x = "Temperature",
  y = "log10 Enterococci", 
  legend = "Season"
)
